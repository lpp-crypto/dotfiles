...
#+BEGIN_SRC elisp
  (defun pi2-6/open-slide-show()
    "Assuming the attached data is a folder containing pictures, starts a slideshow with all of those."
    (shell-command (format "sxiv -r -S 3 -p -sf '%s' &" (org-attach-dir))))
#+END_SRC 

** Attach enhancement
Attachments are, by default, not openable with a simple mouse click. Furthermore, the corresponding headings don't look like links. This is easily fixed by the following function which can turns a heading with an attachment into a link opening it. This is easily done by having the link execute the =org-attach-open= function.

Then, this function is added as an advice to =org-attach= so that it is done automatically every time.

It doesn't work perfectly if there is a ':' in the heading, but it's rare enough and easy enough to fix by hand that I don't care.

#+BEGIN_SRC elisp
  (defun pi2-6/attach-make-link-open()
    (interactive)
    (end-of-line)
    (re-search-backward "^\\(\\*+\\) +\\([^:]+\\) +:")
    (replace-match "\\1 [[elisp:(org-attach-open)][\\2]]  :"))

  (defun pi2-6/attach-make-link-slides()
    (interactive)
    (end-of-line)
    (re-search-backward "^\\(\\*+\\) +\\([^:]+\\) +:")
    (replace-match "\\1 [[elisp:(pi2-6/open-slide-show)][\\2]]  :"))


  (defun pi2-6/attach-make-link()
    (if (member "images" (org-get-tags))
        (pi2-6/attach-make-link-slides)
      (pi2-6/attach-make-link-open)))
...
#+BEGIN_SRC emacs-lisp
  ...
  corresponding to org-roam nodes with the ongoingWork tag."
    ...
    (setopt
     org-agenda-files
     (mapcar 'org-roam-node-file (seq-filter
                                  (lambda (node)
                                    (and
                                          ; the node is indeed a file
                                     (= 0 (org-roam-node-level node))
                                          ; it is tagged "ongoingWork"
                                     (member "ongoingWork" (org-roam-node-tags node))))
                                  (org-roam-node-list)))))
...
#+BEGIN_SRC emacs-lisp
  (defun pi2-6-reset-keys()
    ...
    (pi2-6-global-set-key (kbd "C-! l") 'pi2-6/org-roam-find-library)
    (pi2-6-global-set-key (kbd "C-! r") 'pi2-6/org-roam-find-review)
    (pi2-6-global-set-key (kbd "C-! o") 'pi2-6/org-roam-find-other)
    (pi2-6-global-set-key (kbd "C-! m") 'pi2-6/org-roam-find-meuporg)
    (pi2-6-global-set-key (kbd "C-! c") 'org-roam-capture)
...
#+END_SRC
* Wrap up
The explicit call to =package-initialize= seems to ensure that all packages are properly downloaded if they are absent.
