    ...
    (s-concat
     (when (< 1 (length split-path))
       (propertize (s-concat
                   ...
                   'face '(:weight semi-light :foreground "light gray" :background "dark slate gray")))
     (propertize (s-concat "/" (car (last split-path)) " ")
                 'face '(:weight bold :foreground "white" :background "dark slate gray"))))
     ...
     (cond 
      ...
      ((string-match "\\*.*\\*" (buffer-name))
       (propertize (s-concat " " (buffer-name) " ")
                   'face '(:weight bold :foreground "white" :background "gray")))
