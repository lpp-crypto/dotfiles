...
** Input Configuration
*** Keyboard
A regular keyboard (in particular, a laptop keyboard) must be setup in azerty and =capslock= is set to be another =ctrl=. The specific layout is =fr(oss)= in order to have the "espace insécable fine" and upper-case accented letters in the correct position. Run =man xkeyboard-config= for a list of options.
...
#+BEGIN_SRC sh
bindsym $mod+9 exec python ~/dotfiles/scripts/split-layout.py
...
bindsym XF86AudioRaiseVolume exec sh ~/new-emacs/scripts/sound-change "+5%"
bindsym XF86AudioLowerVolume exec sh ~/new-emacs/scripts/sound-change "-5%"
#+END_SRC
