...
#+BEGIN_SRC emacs-lisp 
(setopt initial-buffer-choice (lambda () (pi2-6/dashboard))
...
** Appearance
*** Font
Using [[https://github.com/tonsky/FiraCode/wiki/Emacs-instructions][fira code]] because it looks nice, alonk with [[https://github.com/source-foundry/Hack][Hack]] because Fira code doesn't have italics. Activating the ligatures of =Fira Code= didn't work---at least not yet. The =ligature= package didn't help.
...
(setopt org-tag-faces
        '(
          ...
          ("review" . (:background "gray" :foreground "white"))
          ("short" . (:background "dark orange" :foreground "white"))
          ("orga" . (:background "tan4" :foreground "white"))
          ("bullshit" . (:background "black" :foreground "white"))
          ("research" . (:background "chartreuse4" :foreground "white"))
          ("programming" . (:background "blue" :foreground "white"))

          
          ("mine" . (:background "black" :foreground "white" :distant-foreground "black"))
          ("sok" . (:background "white" :foreground "red"))

          ("sym" . (:background "sandy brown" :foreground "black" :distant-foreground "black" ))
          ("math" . (:background "tan" :foreground "black"))
          ("mode" . (:background "magenta" :foreground "black"))
          ("advPro" . (:background "sienna" :foreground "black"))

          ("stap" . (:background "blue" :foreground "white" :distant-foreground "white"))
          ("light" . (:background "cyan" :foreground "white" :distant-foreground  "sienna"))
...
starts a slideshow with all of those."
  (call-process-shell-command (format "sxiv -r -S 3 -p -sf '%s' &" (org-attach-dir))))
#+END_SRC 
...
Some more tweaks are needed though, in particular we need to tell org-mode to use =python3= rather than =python= as a command. This is done as [[https://emacs.stackexchange.com/questions/28184/org-mode-how-can-i-point-to-python3-5-in-my-org-mode-doc][here]]. We also tell org-mode not to ask for a confirmation everytime we evaluate a code block.
#+BEGIN_SRC emacs-lisp
...
The capture templates are intended to send the notes in the correct folders, and of course to populate the notes with the correct template. At this stage, =roam= has the following subdirectories:
...
- experiments :: small literate programs intended to try something small (it may or may not become bigger later).
- tasks :: Description of a tiny task (mail, etc)
...
(defun pi2-6/capture-review(title)
  ...
  (vulpea-visit (vulpea-create (format "%s (review)" title)
                               ...
                               :tags '("ongoingWork" "review")
                               :body "- DEADLINE :: \n\n* Files\n** main pdf\n* Diary  :noexport:"
                               )))


(defun pi2-6/capture-experiment(title)
  "Capture an experiment"
  (interactive "sExperiment title: ")
  (vulpea-visit (vulpea-create (format "%s" title)
                               "experiments/${timestamp}_${slug}.org"
                               :tags '("ongoingWork" "experiment")
                               :body (format "\n- DATE :: [%s]\n\n* Notebook\n#+BEGIN_SRC\n\n#+END_SRC\n\n* Diary  :noexport:" (format-time-string
                                                                                                                                (car org-timestamp-formats) (current-time))))))


(defun pi2-6/finalize-current-note()
  "Sets a finished date and adjusts the tags to state the current note does not require any further work"
  (interactive)
  (vulpea-buffer-meta-set
   "FINISHED"
   (format "[%s]"
           (format-time-string (car org-timestamp-formats) (current-time))))
  (vulpea-buffer-tags-remove "ongoingWork")
  (vulpea-buffer-tags-add "stable"))
...
(defun pi2-6/vulpea-nuke-db()
  "Clears the vulpea database and recompute it from scratch"
  (interactive)
  (vulpea-db-clear)
  (vulpea-db-sync-full-scan))
#+END_SRC
...
(transient-define-prefix pi2-6/vulpea-action()
  ...
  [("f" "[f]find" vulpea-find)
   ...
   ("e" "new [e]xperiment" pi2-6/capture-experiment)
   ...
   ("z" "finali[z]e current note" pi2-6/finalize-current-note)
   ("u" "[u]pdate db for this file" vulpea-db-sync-update-file)
   ("n" "[n]uke db and recompute it" pi2-6/vulpea-nuke-db)
   ])
#+END_SRC
...
;;           ("\\.pdf\\'" "evince")
;;           ))
;; (mapcar (lambda (assoc) (list (car assoc) `(format (cdr ,assoc) file)))
;;        file-apps))

(defun pi2-6/open-externally-maybe (filename &rest _)
  "Checks if the file extension is in org-file-apps, and
if yes then runs the corresponding program."
  (when-let ((match (seq-find (lambda (assoc)
                                (string-match-p (car assoc) filename))
                              file-apps)))
    (call-process-shell-command (s-concat (format (cdr match) filename) " &"))
    t))

(advice-add 'find-file :before-until #'pi2-6/open-externally-maybe)
#+END_SRC
...
(use-package consult
  ...
  :bind (;; C-c bindings in `mode-specific-map'
         ...
         ("M-S-v" . consult-yank-pop)                ;; orig. yank-pop
...
*** Python Snippets
**** Main function
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/python-mode/main
# name: __main__
# --
...
#+END_SRC
**** Standard imports
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/python-mode/imports
# name: imports
# --
...
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/python-mode/def
# name: def
# --
def ${1:function_name}(${2:params}) -> ${3:output_type}:
"""!TODO! write docstring for $1
...
#+END_SRC
**** Class declaration
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/python-mode/class
# name: class
# --
class ${1:className}:
"""!TODO! write docstring for class $1
"""

def __init__(self, ${2:params}):
"""!TODO! write docsting for init of class $1
...
#+END_SRC
*** org-mode snippets
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/org-mode/elisp-src
# name: elisp-src
# --
,#+BEGIN_SRC elisp :tangle no
$1
...
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/org-mode/sage-src
# name: sage-src
# --
,#+BEGIN_SRC python :python sage
$1
,#+END_SRC
#+END_SRC
*** C++ snippet
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/c++-mode/docstring
# name: docstring-cc
# --
/** @brief $0
,*
...
#+BEGIN_SRC text :tangle ~/.emacs.d/snippets/c++-mode/for-range
# name: for-range
# --
for (${1:type} ${2:variable}=0; $2<${3:upperBound}; $2++)
{
$0
}
#+END_SRC
...
*** Adding/going back to an anchor
It is common to do something at a specific point, but to want to
quickly visit another part of the file. The following functions allow
placing an "anchor" at the current point to which we can quickly back,
wherever we went to in the file.

#+BEGIN_SRC emacs-lisp
(defun pi2-6/save-anchor()
  (interactive)
  (point-to-register 16449)
  (message "anchor placed"))

(defun pi2-6/back-to-anchor()
  (interactive)
  (jump-to-register 16449)
  (message "back to anchor; it was erased"))
#+END_SRC
*** Avy
Documentation: [[https://github.com/abo-abo/avy]]

Relevant blog post: https://karthinks.com/software/avy-can-do-anything/
...
(defun pi2-6-reset-keys()
  ...
  (pi2-6-global-set-key (kbd "M-\" l") 'avy-goto-line)
  (pi2-6-global-set-key (kbd "M-\" j") 'avy-goto-char-timer)
  ...
  (pi2-6-global-set-key (kbd "C--") 'yas-insert-snippet)
...
** Specific tools
*** dot
Dot is a language to make graphs (as in, nodes and edges). The following use-package call comes from its [[https://github.com/ppareit/graphviz-dot-mode][documentation]].
#+BEGIN_SRC elisp
...
#+END_SRC
** Spelling
In order for ispell to work correctly (and thus flyspell), it is necessary that the files in [[~/new-emacs/aspell/]] are pointed to by symbolic links in the home folder that have the same name (except for a '.' at the start to hide them).

#+BEGIN_SRC elisp :tangle yes :results output silent
(defun pi2-6-start-flyspell-mode(dict)
(ispell-change-dictionary dict)
(flyspell-mode 1)
(flyspell-buffer)
(message (format "launched flyspell; language: %s"
ispell-local-dictionary)))

(defun pi2-6-flyspell-mode(b)
"Prompts the user for a letter and generates a file of the corresponding type"
(interactive "c[e] English   [f] French")
(cond ((eq b ?e) (pi2-6-start-flyspell-mode "english"))
((eq b ?f) (pi2-6-start-flyspell-mode "french"))
(t (message "unrecognized language"))))
...
*** Deadlines
#+BEGIN_SRC elisp
(defun pi2-6/dashboard-parse-project-with-deadline(proj)
  "Returns a list consisting of the number of days left and a formatted
string describing the amount of days left."
  (let* ((timestamp-string (vulpea-meta-get proj "DEADLINE"))
         (days-left (org-timestamp-to-now timestamp-string)))
    (list
     :index days-left
     :prefix
     (s-concat
      (propertize
       (s-concat timestamp-string (format " (in %3d days) " days-left))
       'face (cond ((< days-left 0) 'org-warning)
                   ((< days-left 3) 'org-todo)
                   ((< days-left 8) 'org-upcoming-deadline)
                   ((< days-left 20) 'org-upcoming-distant-deadline)
                   (t '(:foreground "tan"))))
      (propertize "|" 'face 'bold))
     :proj proj)))


(defun pi2-6/dashboard-deadlines ()
  "Inserts widgets to access notes corresponding to ongoingWork that also
have a deadline."
  (pi2-6/dashboard-header "Deadlines" 1)
  (let ((annotated-proj (mapcar 'pi2-6/dashboard-parse-project-with-deadline
                                (seq-filter (lambda (x) (and
                                                         (vulpea-meta-get x "DEADLINE")
                                                         (length> (vulpea-meta-get x "DEADLINE") 4)))
                                            (vulpea-db-query-by-tags-every '("ongoingWork"))))))
    (sort
     annotated-proj
    ...
    (dolist 
        (x annotated-proj)
      (let* ((prefix (plist-get x :prefix))
             (proj (plist-get x :proj))
             (proj-tags (remove "ongoingWork" (vulpea-note-tags proj)))
             (proj-title (vulpea-note-title proj))
             (proj-path (vulpea-note-path proj)))
        (insert prefix)
        (pi2-6/create-file-link proj-title proj-path proj-tags)))))
...
#+BEGIN_SRC elisp
(defvar pi2-6/dashboard-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "q") '(lambda () (interactive) (kill-buffer "*Dashboard*")))
    (define-key map (kbd "o") 'find-file)
    (define-key map (kbd "f") 'vulpea-find)
    (define-key map (kbd "j") '(lambda () (interactive) (avy-goto-char ?→)))
    map)
  "Keymap for the dashboard.")

(set-keymap-parent pi2-6/dashboard-map widget-keymap)
  ...
  (let ((buf (get-buffer-create "*Dashboard*")))
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        ...
        (insert "\n\n")
        (insert (s-concat (propertize "[o]" 'face 'bold)
                          "pen file  ⋅  "
        ...
        (use-local-map pi2-6/dashboard-map)
        (widget-setup)
